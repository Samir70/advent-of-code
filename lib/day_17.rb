class Solution17
  ...
  def run_2(rocks)
    ...
    rocks.times do |i|
      ...
      summary = "rocks mod #{base}: #{(i + 1) % base} => #{tb.summary}"
      if seen[summary] != nil && i + 1 - seen[summary] == base
        # puts tb
        # puts summary
        # puts "rock #{i + 1}, height: #{tb.height}"
        # if not_found_repeat
        ...
        #   height_at_first_rpt = tb.height
        # else
        diff = tb.height - heights[seen[summary]]
        # puts "Rock #{seen[summary]} to rock #{i + 1} height diff is: #{diff}"
        future_rpts = (rocks - (i + 1)) / base
        # puts "need to do this #{future_rpts} more times"
        # puts "which will give us #{future_rpts * base} more rocks and #{future_rpts * diff} more height"
        total_rocks = i + 1 + future_rpts * base
        ...
        rocks_needed = rocks - total_rocks
        # puts "then we will need #{rocks_needed} more rocks"
        height_at_rpt_plus_rocks_needed = heights[seen[summary] + rocks_needed]
        extra_height = height_at_rpt_plus_rocks_needed - height_at_first_rpt
        poss_ans.add(total_height + extra_height)
        # puts poss_ans
        return poss_ans.best_ans if poss_ans.ans_list.keys().length > 20
        # end end of if not_found_repeat
        # not_found_repeat = false
      end
      if i % base == 0
        diff = tb.height - prev
  ...
  def run_3(rocks)
    ...
    rocks.times do |i|
      ...
      if (i + 1) % base == 0
        # puts tb
        puts "rock #{i + 1}, height: #{tb.height}"
        average = tb.height / (i + 1).to_f
        estimate = average * rocks
        puts "average height per rock: #{average}"
        puts "estimate for #{rocks} rocks: #{estimate}"
      end
    end
    puts tb
    return tb.height
